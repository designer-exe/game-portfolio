...
export default class Car
{
    ...
    setChassis()
    {
        ...
        // Restyle car to cream, deep chocolate, and light brown (#B9825A)
        this.chassis.object.traverse((_child) =>
        {
            if(_child instanceof THREE.Mesh)
            {
                if(_child.name.match(/white/i))
                {
                    _child.material = this.materials.shades.items.beige
                }
                else if(_child.name.match(/black/i))
                {
                    _child.material = this.materials.shades.items.brown
                }
                else if(_child.name.match(/red|orange/i))
                {
                    _child.material = this.materials.shades.items.orange
                }
            }
        })
    ...
    setAntena()
    {
        ...
        this.antena.object.traverse((_child) =>
        {
            if(_child instanceof THREE.Mesh && _child.name.match(/red|orange/i))
            {
                _child.material = this.materials.shades.items.orange
            }
        })
    ...
    setWheels()
    {
        ...
        this.wheels.object.traverse((_child) =>
        {
            if(_child instanceof THREE.Mesh)
            {
                if(_child.name.match(/white|metal|gray/i))
                {
                    _child.material = this.materials.shades.items.beige || this.materials.shades.items.white
                }
                else if(_child.name.match(/black/i))
                {
                    _child.material = this.materials.shades.items.brown || this.materials.shades.items.black
                }
            }
        })
        this.wheels.items = []
